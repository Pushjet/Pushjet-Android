...
import io.rubygrapefruit.platform.*;
import io.rubygrapefruit.platform.Process;
import io.rubygrapefruit.platform.internal.DefaultProcessLauncher;
...
public class NativeServices extends DefaultServiceRegistry {
    ...
    public static void initialize(File userHomeDir) {
        ...
        if (useNativePlatform) {
            try {
                io.rubygrapefruit.platform.Native.init(nativeDir);
    ...
    protected ConsoleDetector createConsoleDetector(OperatingSystem operatingSystem) {
        if (useNativePlatform) {
            try {
                Terminals terminals = io.rubygrapefruit.platform.Native.get(Terminals.class);
    ...
    protected ProcessLauncher createProcessLauncher() {
        if (useNativePlatform) {
            try {
                return io.rubygrapefruit.platform.Native.get(ProcessLauncher.class);
            } catch (NativeIntegrationUnavailableException e) {
